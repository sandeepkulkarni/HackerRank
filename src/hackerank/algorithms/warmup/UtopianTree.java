...
public class UtopianTree {
	...
	private static int monsoonCycle(int value){
		value = value * 2;
		return value;
	}
	
	private static int summerCycle(int value){
		value = value + 1;
		return value;
	}

	public static void main(String[] args) {
		System.out.println("--------Start Utopian Tree---------");
		Scanner scanner = new Scanner(System.in);
		int noOfTests = scanner.nextInt();
		int[] cycles = new int[noOfTests]; 
		...
		for(int cycle : cycles){
			...
			int value = 1; 	//initial height is 1 meter
			
			while(cycle > 0){
				value = monsoonCycle(value);
				
				cycle = cycle - 1;			//decrement as a cycle is over
				
				if(cycle > 0){
					value = summerCycle(value);
					cycle = cycle - 1;		//decrement as a cycle is over
				}
